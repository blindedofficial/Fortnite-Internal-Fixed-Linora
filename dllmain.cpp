...
bool EntitiyLoop()
{
	...
	try
	{
		...
		if (valid_pointer(LocalPawn)) {
			LocalRootComponent = read<uintptr_t>(LocalPawn + 0x138);
		...
		for (int i = 0; i < ActorCount; i++) {
			...
			if (valid_pointer(LocalPawn))
			{
				...
				if (Settings::VehiclesESP && strstr(name, xorstr("Vehicl")) || strstr(name, xorstr("Valet_Taxi")) || strstr(name, xorstr("Valet_BigRig")) || strstr(name, xorstr("Valet_BasicTr")) || strstr(name, xorstr("Valet_SportsC")) || strstr(name, xorstr("Valet_BasicC")))
				{
					...
					{
						FZiplinePawnState ZiplinePawnState = read<FZiplinePawnState>(LocalPawn + 0x18D0);
						ZiplinePawnState.bIsZiplining = true;
						ZiplinePawnState.AuthoritativeValue = 1000.f;

						write<FZiplinePawnState>(LocalPawn + 0x18D0, ZiplinePawnState);
					}
...
VOID MenuAndDestroy(ImGuiWindow& window) {
	...
	if (ShowMenu) {
		...
		if (ImGui::Begin(xorstr("linorgay updated by blinded#9923"), &ShowMenu, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse )) {
...
float* CalculateShotHook(PVOID arg0, PVOID arg1, PVOID arg2) {
	...
	if (ret && Settings::SilentAim && LocalPawn && TargetPawn) {
